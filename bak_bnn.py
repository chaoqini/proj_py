...
#        print('init_params:')
        ...
        (nl,params_init,params_bn_init,g,g_d)=([],{},{},[],[])
        nl.append(nx)
        ...
        for i in range(lays):
            ...
            params_bn_init['beta'+str(i)]=np.ones((nl[i+1],1))*0
            params_bn_init['gama'+str(i)]=np.random.randn(nl[i+1],1)*1e-3
            g.append(func)
            g_d.append(func_d)
        g[-1]=softmax;g.append(log_loss)
        params=copy.deepcopy(params_init)
        params_bn=copy.deepcopy(params_bn_init)
        return (params,params_init,params_bn,params_bn_init,g,g_d)
    (params,params_init,params_bn,params_bn_init,g,g_d)=init_params()


## ==========
    def fp_bn(X,params,params_bn,g,isop=0):
        if X.ndim==2: X=X.reshape(tuple([1])+X.shape)
        assert(X.ndim==3)
		e=1e-8
        (l,OP)=(int(len(params)/2),{})
        OP['A-1']=X
        for i in range(l):
			wi=params['w'+str(i)]
			bi=params['b'+str(i)]
			gamai=params_bn['gama'+str(i)]
			betai=params_bn['bata'+str(i)]
			Ai_1=OP['A'+str(i-1)]
			Zi=wi@Ai_1+bi
...
## ==========
    def bp_bn(X,LAB,params,params_bn,g,g_d):
        ...
        for i in range(l-1,-1,-1):
			...
			d_Xi=gamai*d_Yi
			mi=Xi.shape[1]
			dXi_Zi=(mi*np.eye(mi)-np.ones((mi,mi))-Xi@Xi.transpose(0,2,1))/(mi*vi**0.5)
			d_Zi=dXi_Zi.transpose(0,2,1)@d_Xi
			d_Ain1=wi.tranpose(0,2,1)@d_Zi
			...
			grad['d_beta'+str(i)]=d_batai.mean(axis=0)
        return grad
## ==========
    def bp(X,LAB,params,g,g_d):
        ...
        return grad
## ==========
    def slope(x,lab,params,parama_bn,g,dv=1e-5):
#        print('slope:')
        slp={}
        pt=copy.deepcopy(params)
        pt_bn=copy.deepcopy(params_bn)
        for (k,v) in pt.items():
...
#            iseq=iseq&(np.any(pt[k]==params[k])) 
            ...
            iseq=iseq&(np.all(pt_bn[k]==params_bn[k])) 
        assert(iseq==1)
        return slp
## ==========
    def grad_check(x,lab,params,params_bn,g,g_d,dv=1e-5):
#        print('grad_check:')
        y1=bnn.bp_bn(x,lab,params,params_bn,g,g_d)
        y2=bnn.slope(x,lab,params,params_bn,g,dv)
...
## ==========
    def update_params(params,params_bn,grad,lr=1e-3):
        for k in params.keys():
            params[k]-=lr*grad['d_'+k]
        for k in params_bn.keys():
            params_bn[k]-=lr*grad['d_'+k]
        return params
    def update_params_adam(params,params_bn,grad,lr,v,s,t=1):
        ...
        for k in params_bn.keys():
            ...
            sc_k = s[k]/(1-beta2**t)
            adam_d_k = vc_k/(sc_k**0.5+e)
            params_bn[k]-=lr*adam_d_k
        return (params,v,s)
    def init_adam(params,params_bn):
        (v,s)=({},{})
        for k in params.keys():
            v[k] = np.zeros(params[k].shape)
            s[k] = np.zeros(params[k].shape)
        for k in params_bn.keys():
            v[k] = np.zeros(params_bn[k].shape)
            s[k] = np.zeros(params_bn[k].shape)
        return (v,s)
## ==========
    def normalize(X):
#        AX=mnist.train_img
