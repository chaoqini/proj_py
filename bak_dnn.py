...
## ==========
class dnn:
    def init_params(lays=3,nnode=100,nx=0,ny=0,func=0,seed=0):
        print('init_params:')
        np.random.seed(seed)
        if nx==0: nx=28*28
        if ny==0: ny=10
        ...
        nl.append(nx)
        for i in range(lays): nl.append(nnode)
        nl.append(ny)
        for i in range(lays):
            bi=('b'+str(i))
            wi=('w'+str(i))
...
## ==========
#    def fp(X,params=dnn.params,g=dnn.g,isop=0):
    def fp(X,params,g,isop=0):
        ...
        OP['A-1']=X
#        print('fp: g=',g)
#        print('fp: len(params)=',len(params))
#        print('fp: params.keys()=',params.keys())
#        print('fp: l=',l)
#        print('fp: len(g)=',len(g))
        for i in range(l):
#            print('fp: i=',i)
#            print('fp: g[i]=',g[i])
        ...
        if isop==0: 
            ...
            return (Y,OP)
## ==========
#    def bp(X,LAB,params=dnn.params,g_d=dnn.g_d):
...
def batch_train(params,g,g_d,lr0=0.1,klr=0.998,batch=20,batches=0,isplot=0,istime=0):
#    if batch<1: batch=dnn.batch
    ...
    print('Train type(params)=',type(params))
    for k in params.keys():
...
def dnn_train(params,g,g_d,lr0=0,klr=0,batch=0,batches=0,isplot=0,istime=0,ischeck=0):
...
#def batch_train(params,g,g_d,lr0=0,klr=0,batch=0,batches=0,isplot=0,istime=0):
...
#(params,params_init,g,g_d,lr0,klr,batch)=init_dnn()
#def init_dnn(lays=0,nnode=0,nx=0,ny=0,f=0,seed=0,lr0=0,klr=0,batch=0,isonlyparams=0):
def hyperparams_test(params,g,g_d,nloop=0,nnode=0,lr0=0,klr=0,batch=0,batches=0):
    print('heyperparams_test:')
#    params=dnn.init_dnn(lays=lays,nnode=nnode,isonlyparams=1)
#    if lays==0: lays=3
    if nloop==0: nloop=5
#    if lr0==0: lr0=0.01
#    if klr==0: klr=1
...
#        if lr0!=0:
#            j=10**(-i/10*1)*lr0
#            (v1,v2)=dnn_train(params,lr0=j,klr=0.998,batch,batches,1)
#        if klr!=0
#        (v1,v2)=dnn_train(params,lr0=0.003,klr=1-0.001*i,batch,batches,1,0)
...
#def dnn_train(params,lr0=0,klr=0,batch=0,batches=0,isplot=0,istime=0,ischeck=0):
        lri=lr0*(1-i/nloop)
#        klr=klr-0.001*i
#        (v1,v2)=dnn_train(params,lr0i,klri,batchi,batches,isplot=1)
#def dnn_train(params,g,g_d,lr0=0,klr=0,batch=0,batches=0,isplot=0,istime=0,ischeck=0):
        (v1,v2)=dnn_train(params,g,g_d,lri,klr,batch,0,1,0,0)
        v1a.append(v1)
        v2a.append(v2)
    v1a=np.array(v1a)
    v2a=np.array(v2a)
...
#hyperparams_test(nloop=3,lays=2)
#dnn.hyperparams_test(nloop=6,lays=5,nnode=200,lr0=0.003,klr=0.999,batch=30)
#params=dnn.init_dnn(lays=6,nnode=200,isonlyparams=1)
#def hyperparams_test(params,g,g_d,nloop=0,lays=0,nnode=0,lr0=0,klr=0,batch=0,batches=0):
#def init_dnn(lays=0,nnode=0,nx=0,ny=0,f=0,seed=0,lr0=0,klr=0,batch=0,isonlyparams=0):
#(params,params_init,g,g_d,lr0,klr,batch)=dnn.init_dnn(lays=6,nnode=200)
